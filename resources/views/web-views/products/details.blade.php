...
@section('content')
    ...
    <div class="container mt-4 rtl" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
        <div style="text-align:center"><strong>Coupon code R69567</strong></div>
        <!-- General info tab-->
                                    ...
                                    @foreach (json_decode($product->images) as $key => $photo)
                                        <div
                                            ...
                                            @if($product->file !== null)
                                                <img class="img-responsive" style="width:100%;max-height:323px;"
                                                     data-id="{{$product->id}}"
                                                     onclick="pdfClick(event.target)"
                                                     data-toggle="modal"
                                                     type="button"
                    ...
                    <div class="col-lg-7 col-md-8 col-12 mt-md-0 mt-sm-3" style="direction: {{ Session::get('direction') }}">
                        <div class="details">
                            ...
                            <form id="add-to-cart-form" class="mb-2">
                                @csrf
                                <input type="hidden" id="productIdValue" name="id" value="{{ $product->id }}">
                            ...
                            <div class="d-flex justify-content-start mt-2 mb-3">
                                <button
                                    class="btn btn-info element-center btn-gap-{{Session::get('direction') === "rtl" ? 'left' : 'right'}} fa fa-book m-2"
                                    data-id="{{$product->id}}"
                                    onclick="pdf(event.target)"
                                    data-toggle="modal"
                                    type="button"
                                    style="width:37%; height: 45px; !important; color: #ffffff;">
                                    <span class="string-limit"> {{\App\CPU\translate('Ebook')}}</span>
                                </button>
                                <button
                                    class="btn btn-success element-center btn-gap-{{Session::get('direction') === "rtl" ? 'left' : 'right'}}"
                                    onclick="audioBook()"
                                    type="button"
                                    style="width:37%; height: 45px;{{Session::get('direction') === "rtl" ? 'margin-right: 20px;' : 'margin-left: 20px;'}}">
                                    <span class="string-limit"><i class="fa fa-volume-up m-2"></i>{{\App\CPU\translate('Audio Book')}}</span>
                                </button>
                            </div>
    ...
    <div id="editCoupon111" class="modal fade" role="dialog">
    ...
    <div id="editCoupon" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div>
                        <img style="width: 100%" src="{{asset('public/assets/front-end/img/coming.jpg')}}" alt="">
                    </div>
                </div>
            </div>
        </div>
        ...
        function pdfClick(event) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var pdfPath = '{{ asset("public/assets/front-end/pdf/") }}/' + file;

                    // Update the src attribute of the embed element with the dynamic file path
                    $('#pdffile').html('<embed src="' + pdfPath + '" frameborder="0" width="100%" height="400px">');

                    $('#editCoupon111').modal('show');
                }
            });
        }
